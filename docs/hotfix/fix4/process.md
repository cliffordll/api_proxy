...
| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| 协议常量扩展 | `common/routes.py` 新增 `ROUTE_PRIORITY` + `DEFAULT_MOCKUP_ROUTES` | `common/routes.py` | 完成 |
...
## Phase 2 — Server 默认 mockup

| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| Server 默认 mockup | `DEFAULT_CONFIG` 引用 `DEFAULT_MOCKUP_ROUTES`，三条全 mockup | `app/core/loader.py` | 完成 |
...
## Phase 3 — Mockup 文案个性化

| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| Mockup 文案 | `[mockup] {model} speaking ({协议}). How can I help?` | `app/clients/mockup_client.py` | 完成 |
...
## Phase 4 — CLI 读 routes 回退

| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| CLI 读 routes 回退 | `load_routes` 在 yaml 无 routes 时回退到 `DEFAULT_MOCKUP_ROUTES` | `cli/core/config.py` | 完成 |

**执行记录**：
- `cli/core/config.py::load_routes` 在 yaml 没有 routes 段时，返回 `{**DEFAULT_MOCKUP_ROUTES}`
- server 和 CLI 在"无配置"时看到同一份 mockup routes，展示统一
...
| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| probe 结构化 | 新增 `probe_route` / `probe_all`，返回 `{status, status_reason, models, ...}` | `cli/core/probe.py` | 完成 |

**执行记录**：
- `probe_route(name, conf)` 返回 `{route, provider, base_url, status: "ok"|"failed"|"mockup", status_reason, models}`
- mockup 路由跳过实际 HTTP 探测
- `probe_all` 用 `asyncio.gather` 并发
...
| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| Display 扩展 | 新增 `_format_route_sections` / `print_welcome` 改版 / `print_route_status` / `print_route_picker`；强制 UTF-8 避免 Windows GBK 编码失败 | `cli/core/display.py`、`main.py` | 完成 |
...
| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| chat 启动决策 | `_startup_probe` 决定路由 + 并发探测；Repl 接收 route_results + footer_note；`_refresh_current_route` 单路由刷新 | `cli/repl.py` | 完成 |
...
| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| `/routes` 命令 | 数字 picker，选中后走 `/route <name>` 流水线 | `cli/chat/commands.py` | 完成 |
...
| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| Tab 补全 + /help | `/routes` 加入 COMMANDS；帮助文案同步 | `cli/repl.py`、`cli/chat/commands.py` | 完成 |
...
| 任务 | 说明 | 产出文件 | 状态 |
|------|------|---------|------|
| 测试 + E2E | pytest 85 passed；默认 / 显式 --base-url 启动 E2E 验证 | — | 完成 |
